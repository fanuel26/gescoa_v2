...
const columns = [
  ...
  },
  {
    ...
    dataIndex: "nom_collecteur",
    key: "nom_collecteur",
  },
  {
    title: "Code carnet",
    dataIndex: "nom_carnet",
    key: "nom_carnet",
  },
  {
    title: "Cotisation controle (Fcfa)",
    dataIndex: "cotisation_c",
    key: "cotisation_c",
  },
  {
    title: "Cotisation réelle (Fcfa)",
...
export default {
  ...
  methods: {
    ...
    listDemande() {
      ...
      this.$http
        .get(
          `${this.callback}/make-controle/all/controlleur/${this.$route.params.id}`,
          headers
        )
        .then(
          (response) => {
            let data = response.body.controles;
            ...
            for (let i = 0; i < data.length; i++) {
              ...
              this.data.push({
                key: data[i]._id,
                created_at: new Date(data[i].createdAt).toLocaleString(),
                nom_collecteur: `${data[i].carnet.collecteur.nom} ${data[i].carnet.collecteur.prenoms}`,
                nom_carnet: data[i].carnet.uuid,
                // nbr_mois: data[i].id_mois,
                // nbr_jours: data[i].nbr_jours,
                dette: data[i].montantDette,
                cotisation_c: data[i].nbrCotisation * data[i].carnet.mise,
                cotisation: data[i].nbrCotisationCollecteur * data[i].carnet.mise,
                etat: data[i].validateDette,
              });
            }
          },
          (response) => {
    ...
    resolve(id) {
      ...
      this.$http
        .put(
          `${this.callback}/make-controle/dette/controlleur/${this.$route.params.id}/controle/${id}`, {},
          headers
        )
        .then((response) => {
          ...
          if (data.status == 200) {
            ...
            this.showAlert(
              "danger",
              "Erreur",
              "Probleme non resolue suite une erreur"
            );
          }
        });
    },
