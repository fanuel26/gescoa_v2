        ...
        <a-card class="card card-body border-0">
          ...
          <div class="col-info">
            <a-descriptions
              :title="
                'date de création: ' +
                new Date(controlleur.created_at).toLocaleString()
              "
              :column="3"
            >
              ...
              <a-descriptions-item label="Nom agence">
                {{ controlleur.nom_agence }}
              </a-descriptions-item>
            </a-descriptions>
          </div>
          <div class="my-4" >
            <a-popconfirm v-if="controlleur.email !== 'admin@gmail.com'"
              v-bind:title="'Sûre de modifier le mot de passe? Mot de passe: ' + password "
...
export default {
  ...
  mounted() {
    this.password = `testfood@${Math.floor(Math.random() * (9999 - 1000) + 1000)}`;

    this.stats = [
      {
        title: "Nombre de collecteur",
        value: 0,
        prefix: "",
        suffix: "",
  ...
  methods: {
    ...
    detailCollecteur() {
      this.controlleur = JSON.parse(localStorage.getItem("dataController"));
      console.log(this.controlleur);
    },
