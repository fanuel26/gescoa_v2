...
export default {
  ...
  methods: {
    ...
    listDemande() {
      ...
      this.$http.get(`${this.callback}/controlleur/allByAdmin`, headers).then(
        (response) => {
          let data = response.body.controlleurs;
          ...
          for (let i = 0; i < data.length; i++) {
            this.data.push({
              ...
              nom_agence: data[i].agence.libelle,
              nom: `${data[i].nom} ${data[i].prenoms}`,
              email: data[i].email,
              telephone: data[i].telephone,
              code_secret: data[i].codeSecret,
    ...
    passwordSubmit(id) {
      ...
      this.$http
        .put(
          `${this.callback}/controlleur/generateMdpControlleur/${id}`,
          data_param,
          headers
        )
        .then(
    ...
    chefSubmit(data) {
      ...
      this.$http
        .post(`${this.callback}/controlleur/create`, dataSubmit, headers)
        .then((response) => {
          ...
          if (response.body.status == 200) {
            this.showAlert(
              "success",
              "Compte Créer",
              "Controlleur creer avec success"
            );
            this.listDemande();
          }
        });
