        ...
        <a-card class="card card-body border-0">
          ...
          <a-table :columns="columns" :data-source="data">
            <!-- <template slot="operation" slot-scope="text, record">
              <div class="d-flex">
                <a-popconfirm
                  title="Sûre de livrés?"
                  cancel-text="annuler"
                  ok-text="Valider"
                  @confirm="() => block(record.key)"
                  ><a-button class="mx-2" size="small">Activer</a-button>
                </a-popconfirm>
              </div>
            </template> -->
          </a-table>
...
export default {
  ...
  mounted() {
    ...
    this.stats = [
      {
        title: "Nombre de carnets desactiver",
        value: 0,
        prefix: "",
        suffix: "",
  ...
  methods: {
    listeCarnet() {
      ...
      this.$http
        .post(`${this.callback}/carnet/exemplaire/list/blocked`, {}, headers)
        .then(
          (response) => {
