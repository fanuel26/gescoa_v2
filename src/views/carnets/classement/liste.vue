                ...
                <a-card class="card card-body border-0">
                    <div class="mb-4 d-flex justify-content-between align-items-center">
                        <a-input-search v-model="value" placeholder="Recherche ici" style="width: 300px"
                            @change="onSearch" />
                        <div>
                            <!-- <a-range-picker v-model="valueDate" :format="dateFormat" @change="listeCarnet" /> -->
                  ...
                  Envoyer
                    ...
                    <a-table :columns="columns" :data-source="data">
                        <template slot="operation" slot-scope="text, record">
                            <div class="d-flex">
                                <a-button v-if="record.status == 1" type="primary" class="mx-2" size="small">Deja livrés
                                </a-button>

                                <a-popconfirm v-if="record.status == 0" title="Sûre de livrés?" cancel-text="annuler"
...
export default {
    ...
    methods: {
        listeCarnet() {
            ...
            this.$http
                .post(`${this.callback}/carnet/exemplaire/list/sort`,
                    { }, headers)
                .then(
                    (response) => {
                        ...
                        if (data.length == 0) {
                            this.showAlert('warn', 'Oups!', 'Vous n\'avez pas de carnet terminer dans cette periode')
                        }
