      ...
      <a-col :span="24" :md="16" class="mb-24">
        ...
        >
          ...
          <a-col :span="24" :md="24" class="mb-24">
            ...
            >
              <template #title>
                <h6 class="font-semibold m-0">Liste des caissier affectés</h6>
              </template>
              <a-table
                :columns="columns_caisse"
                :data-source="data_caisse"
                :pagination="true"
        ...
        .then(
          (response) => {
            ...
            if (response.status == 200) {
              ...
              this.stats[0].value = data.totalCollecteur;
              this.stats[1].value = data.totalAgent;
              this.stats[2].value = data.sommeTotalDeversements;
            }
          },
          (response) => {
            this.showAlert("error", "Erreur", response.body.message);
    ...
    listeCaissier() {
      ...
      this.$http.get(`${this.callback}/agent/all`, headers).then(
        (response) => {
          ...
          console.log(data);
          for (let i = 0; i < data.length; i++) {
            if (data[i].agence.id == this.$route.params.id) {
              this.data_caisse.push({
                key: data[i].id,
