        ...
        <a-card class="card card-body border-0">
          <template #title>
            <div class="d-flex justify-content-between align-items-center">
              <h6>Classement des plus gros clients</h6>
              <a-form
                id="components-form-demo-normal-login"
                :form="form"
                class="normal_login pt-2"
                @submit="classementSubmit"
                :hideRequiredMark="true"
              >
                <a-form-item>
                  <a-input
                    placeholder="Nombre de carnet"
                    v-decorator="[
                      'nbr',
                      {
                        initialValue: null,
                  ...
                  <a-button type="primary" htmlType="submit" class="mx-4">
                    Envoyer
                  </a-button>
                </a-form-item>
              </a-form>
...
export default {
  ...
  methods: {
    ...
    listeClient() {
      ...
      this.$http
        .post(
          `${this.callback}/v2/client/carnet-count/0/by-collector/${this.$route.params.id}?all=true`,
          {},
          headers
        )
        .then(
    ...
    classementSubmit(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          // parametre: $valuer_nombre_carnet=2 $collecteur=36
          // http://egal.iziway.tk/api/auth/admin/v2/client/carnet-count/2/by-collector/36
          this.$http
            .post(
              `${this.callback}/v2/client/carnet-count/${values.nbr}/by-collector/${this.$route.params.id}?all=true`,
              {},
              headers
            )
            .then(
