      ...
      <a-col :span="12" :lg="12" :xl="24" class="mb-24">
        <a-card class="card card-body border-0">
          ...
          <div class="d-flex justify-content-between align-items-center mb-24"
            ...
            <div>
              <a-date-picker :value="dataMonth" placeholder="Select month" picker="month" mode="month"
                @panelChange="getListMonth" />
            </div>
            <div>
...
export default {
  ...
  methods: {
    ...
    getListMonth(value) {
      ...
      let date = new Date(value._d).getMonth()
      console.log(date)
      console.log(data)
      this.data = [];
      ...
      for (let i = 0; i < data.length; i++) {
        let createdAt = data[i].createdAtMonth
        
        if (date == createdAt) {
          this.data.push(data[i]);

          this.stats[1].value += parseInt(data[i].somme)
        }

        this.stats[2].value += parseInt(data[i].somme)
      }
    },
      ...
      this.$http
        ...
        .then(
          (response) => {
            ...
            for (let i = 0; i < data.length; i++) {
              let createdAt = new Date(data[i].createdAt)
              this.data.push({
                key: data[i].id,
              ...
              if (nowMonth == createdAt.getMonth()) {
                console.log(nowMonth)
                console.log(createdAt.getMonth())
                this.stats[1].value += data[i].montant
              }
              this.stats[2].value += data[i].montant
    ...
    onSearch() {
      ...
      this.data = [];
      for (let i = 0; i < data.length; i++) {
        let somme = data[i].somme.toLowerCase().indexOf(this.value);
